...
public class HuaweiAiGalleryChatClient
        extends AbstractFunctionCallSupport<GalleryChatChunk, GalleryChatReq, ResponseEntity<GalleryChatResp>>
    ...
    public HuaweiAiGalleryChatClient(LLM llm) {
        this(llm, HuaweiAiGalleryChatOptions.builder()
    ...
    public HuaweiAiGalleryChatClient(LLM llm, HuaweiAiGalleryChatOptions options) {
        this(llm, options, null, RetryUtils.DEFAULT_RETRY_TEMPLATE);
    }

    public HuaweiAiGalleryChatClient(LLM llm, HuaweiAiGalleryChatOptions options, FunctionCallbackContext functionCallbackContext, RetryTemplate retryTemplate) {
    ...
    @Override
    protected GalleryChatReq doCreateToolResponseRequest(GalleryChatReq previousRequest, GalleryChatChunk responseMessage, List<GalleryChatChunk> conversationHistory) {
        return null;
    }

    @Override
    protected List<GalleryChatChunk> doGetUserMessages(GalleryChatReq request) {
        return null;
    }

    @Override
    protected GalleryChatChunk doGetToolResponseMessage(ResponseEntity<GalleryChatResp> response) {
        return null;
    }

    @Override
    protected ResponseEntity<GalleryChatResp> doChatCompletion(GalleryChatReq request) {
        return null;
    }
...
// 设置回调处理逻辑
    ...
    ChatRequest createRequest(Prompt prompt, boolean stream) {
        ...
        if (prompt.getOptions() != null) {
            if (prompt.getOptions() instanceof ChatOptions runtimeOptions) {
                var updatedRuntimeOptions = ModelOptionsUtils.copyToTarget(runtimeOptions, ChatOptions.class,
                        HuaweiAiGalleryChatOptions.class);
        ...
        if (!CollectionUtils.isEmpty(functionsForThisRequest)) {
            ...
            request = ModelOptionsUtils.merge(
                    HuaweiAiGalleryChatOptions.builder().withTools(this.getFunctionTools(functionsForThisRequest)).build(),
    ...
    @Override
    protected boolean isToolFunctionCall(ResponseEntity<GalleryChatResp> chatCompletion) {
        ...
        return !StringUtils.hasText(choices.get(0).getText());
    }
}
